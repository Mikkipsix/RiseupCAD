...
def seg(kind, x1, y1, x2, y2, role="thin"):
    return SimpleNamespace(kind=kind, x1=x1, y1=y1, x2=x2, y2=y2, role=role, length=((x2-x1)**2+(y2-y1)**2)**0.5)


def test_horizontal_dimension_links_vertical_witnesses():
    dims = [Dim(500, False, 100, 139, 50, Num(500, 119.5, 50, 27, 12, 90, False))]
...
def test_vertical_dimension_links_horizontal_witnesses():
    dims = [Dim(100, True, 10, 18, 40, Num(100, 40, 14, 10, 27, 82, True))]
