...
def build_dims(numbers, segs, reach=9.0, scale=None, diagnostics=False):
    ...
    if scale is None:
        scale = _estimate_scale(numbers, segs)
    out = []
    for nb in numbers:
        ...
        for c in cands:
            ...
            if target:
                c["score"] = (c["target_error"], c["center_error_px"], 0 if c["inside"] else 1, c["symmetry"], c["span_px"])
            else:
                # Dimension text is commonly placed outside the two witness
                # lines. Prefer the pair whose midpoint is nearest to the
                # text, then use containment/symmetry as tie breakers.
                c["score"] = (c["center_error_px"], 0 if c["inside"] else 1, c["symmetry"], c["span_px"])
...
def refine_dims(numbers, segs, scale, rel_tol=0.04, reach=9.0):
    ...
    for nb in numbers:
        ...
        if not valid:
            continue
        best = min(valid, key=lambda c: (abs(c["span_px"] - target) / target, c["center_error_px"], 0 if c["inside"] else 1, c["symmetry"]))
        out.append(Dim(
            nb.value,
            nb.vertical,
            best["a"],
            ...
            meta={
                ...
                "candidate_count": len(cands),
                "selected": best,
                "candidates": sorted(cands, key=lambda c: (abs(c["span_px"] - target) / target, c["center_error_px"], 0 if c["inside"] else 1))[:12],
            },
        ))
    return out
