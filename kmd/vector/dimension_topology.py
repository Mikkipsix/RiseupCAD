    ...
    for di, d in enumerate(dims):
        ...
        for si, s in enumerate(segs):
            ...
            if not (lo - position_tol <= line <= hi + position_tol):
                continue
            end_error = min(abs(pos - a), abs(pos - b))
            if end_error > span_tol:
                continue
            candidates.append({"segment": si, "position": round(pos, 2), "range": [round(lo, 2), round(hi, 2)], "end_error": round(end_error, 2), "role": getattr(s, "role", ""), "length": round(float(getattr(s, "length", 0.0)), 2)})
        ...
        confidence = (0.35 if object_a is not None else 0.0) + (0.35 if object_b is not None else 0.0)
        if target and target > 0:
            confidence += 0.30 * max(0.0, 1.0 - span_error / max(target, 1e-6))
        out.append({"dimension_index": di, "value": int(d.value), "orientation": "vertical" if vertical_dim else "horizontal", "text": {"x": round(float(d.num.x), 2) if d.num else None, "y": round(float(d.num.y), 2) if d.num else None, "w": round(float(d.num.w), 2) if d.num else None, "h": round(float(d.num.h), 2) if d.num else None, "confidence": round(float(d.num.conf), 1) if d.num else None}, "span_px": round(span, 2), "target_px": round(target, 2) if target is not None else None, "span_error_px": round(span_error, 2) if span_error is not None else None, "object_a": object_a, "object_b": object_b, "witness_candidates": candidates[:12], "confidence": round(confidence, 3)})
    return out
