...
# =====================================================================
def _prep(bw):
    img = cv2.resize(255 - bw, None, fx=OCR_SCALE, fy=OCR_SCALE,
...
def ocr_numbers(bw, min_h=7, max_h=70):
    ...
    for v, x, y, w, h, c in _boxes(img):
        found.append(Num(v, (x + w / 2.0 - PAD) / OCR_SCALE,
                         (y + h / 2.0 - PAD) / OCR_SCALE,
                         w / OCR_SCALE, h / OCR_SCALE, c, False))
    for v, x, y, w, h, c in _boxes(cv2.rotate(img, cv2.ROTATE_90_CLOCKWISE)):
        rx, ry = x + w / 2.0, y + h / 2.0
        found.append(Num(v, (ry - PAD) / OCR_SCALE,
                         (H - 1 - rx - PAD) / OCR_SCALE,
                         h / OCR_SCALE, w / OCR_SCALE, c, True))
    hh, ww = bw.shape
    out = []
